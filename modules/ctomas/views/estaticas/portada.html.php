<!DOCTYPE html>

<html lang="es">

<head>
    <link rel="stylesheet" href="/assets/css/portada.css">
</head>

<body>
    <div class="principal">

        <section class="module parallax parallax-1">
            <div class="container punteado">
                <h1>Fast CV</h1>
            </div>
        </section>

        <section class="module content">
            <div class="container">
                <h2 class="title">Tu generador de currículums online</h2>
                <span id="logofastcv"></span>
                <p>Con Fast CV puedes crear tus currículums de manera rápida y sencilla para mostrarlos online o descargarlos en formato pdf. 
                    Es una herramienta completamente gratuita, tan solo es necesario <a href="/usuarios/insert">registrarse</a> 
                    como usuario. </p>
            </div>
        </section>

        <section class="module content">
            <div id="slideshow" class="container">
                <ul>
                    <li id="im1">
                        <div class="container">
                            <div class='punteado'>
                                <h1>Crea tus currículums gratis</h1>
                            </div>
                        </div>
                    </li>

                    <li id="im2">
                        <div class="container">
                            <div class='punteado'>
                                <h1>Comparte tu experiencia laboral</h1>
                            </div>
                        </div>
                    </li>

                    <li id="im3">
                        <div class="container">
                            <div class='punteado'>
                                <h1>Gestiona y edita tus currículums</h1>
                            </div>
                        </div>
                    </li>
                </ul>
            </div>
        </section>

    </div>
    <!-- /principal -->
    
</body>

</html>